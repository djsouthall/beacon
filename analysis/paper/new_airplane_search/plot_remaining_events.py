...
if __name__ == '__main__':
    ...
    filtered_df = df.query('key == "good" and suspected_airplane_icao24 != suspected_airplane_icao24') #Selecting nan values
    runs = filtered_df['run'].to_numpy()
    eventids = filtered_df['eventid'].to_numpy()
    ...
    for i in range(len(runs)):
        ...
        ds.inspector_mpl['fig1_wf_h'].text(0.985, 0.95, 'Run %i Event %i'%(run, eventid), transform=ds.inspector_mpl['fig1_wf_h'].transAxes, fontsize=18, verticalalignment='bottom', horizontalalignment='right', bbox=dict(boxstyle="square", fc="w"))
