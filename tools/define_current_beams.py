...
if __name__=="__main__":
    ...
    '''
    modes = ['vpol_2018', 'hpol_2018']#['vpol_no_ant01']:#['hpol','vpol','vpol_no_ant0','vpol_no_ant01']:

    use_old_pulser = False
    use_pulser=False
    max_method = 0
    ...
    # use None to plot all the beams -- yeah. it's counterintuitive.
    beams_to_plot = None
    # define a list of the beams to plot to make just one.
    #beams_to_plot = [8]
    #beams_to_plot = [0,1,2,4,5,6,8,9,10,12,13,14,16,17,18]
    ...
    deploy_index = 0 # use 0 for 2018; None for 2019

    for hilbert in [False, True]:
        datapath = os.environ['BEACON_DATA']
        
        if use_old_pulser == True:
            run = 792
        ...
        for mode in modes:
            ...
            if False:
                ...
                if mode == 'hpol':
                    ...
                    thetas_deg = numpy.array([-5, 25, 60, 90])
                    phis_per_theta = numpy.array([5,5,5,5,5])
                    phis_angular_ranges = numpy.array([[-50,50], [-50,50], [-50,50], [-50,50], [-50,50], [-50,50]])
                    turnoff_ants = None
                    year = 'any'
                    ...
                    thetas_deg = 90. - numpy.array([-6, 28, 62, 96])
                    phis_per_theta = numpy.array([5,5,5,5,5])
                    phis_angular_ranges = numpy.array([[-40,40], [-40,40], [-40,40], [-40,40], [-40,40], [-40,40]])
                    #thetas_deg = numpy.array([ 0])+90.
                    #phis_per_theta = numpy.array([1])
                    #phis_angular_ranges = numpy.array([[-40,40]])
            ...
            if year == '2018' or '2019':
                # beams are numbered from 0 to 19, increasing in zenith first and then in azimuth.
                # so beam 0 is az = -40, ze = 96, beam 1 az = -40, ze = 62, beam4 is az=-20, ze = 96
                for phi_index, nphis in enumerate(phis_per_theta):                     
                    for theta_index, theta in enumerate(thetas_deg):
                        minphi = phis_angular_ranges[theta_index][0]
                        maxphi = phis_angular_ranges[theta_index][1]
                        dphi = float(maxphi-minphi)/float(nphis - 1)
                        beam_thetas_deg = numpy.append(beam_thetas_deg, thetas_deg[theta_index])
                        beam_phis_deg = numpy.append(beam_phis_deg, minphi + dphi * phi_index)    
            ...
            if beams_to_plot == None:
                beams_to_plot = range(len(beam_thetas_deg))
                
            for beam_index in beams_to_plot:
                print('%i/%i  ze=%2.1f az=%2.1f             '%(beam_index + 1,len(beam_thetas_deg), beam_thetas_deg[beam_index], beam_phis_deg[beam_index]))
                forced_deltats = None 
                if year == '2019':
