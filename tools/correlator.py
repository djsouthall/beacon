...
class Correlator:
    ...
    def mapMax(self, map_values, max_method=0, verbose=False, zenith_cut_ENU=None, zenith_cut_array_plane=None, pol=None, return_peak_to_sidelobe=False, theta_cut=None):
        ...
        zenith_cut_ENU : list of 2 float values
            Given in degrees, angles within these two values are considered.  If None is given for the first then it is
            assumed to be 0 (overhead), if None is given for the latter then it is assumed to be 180 (straight down).
            ***This is ignored if theta_cut is given.
        zenith_cut_array_plane : list of 2 float values
            Given in degrees, angles within these two values are considered.  If None is given for the first then it is
            assumed to be 0 (overhead), if None is given for the latter then it is assumed to be 180 (straight down).  This is
            polarization dependant because it depends on the calibration of the antennas positions.  So if pol=None then this will
            be ignored.
            ***This is ignored if theta_cut is given.
        theta_cut : numpy.ndarray
            This is generated by self.generateThetaCutMask.  This is passable as a parameter such that it is not calculated
            on a per-event basis unless desired.  The user can calculate it once in advance and pass it to this function.
        ...
        try:
            ...
            if return_peak_to_sidelobe:
                blob_label, num_blobs = scipy.ndimage.label(masked_map_values > 0) #Want to change this to something higher.
        ...
        try:
            ...
            for event_index, eventid in enumerate(eventids):
                ...
                if False:
                    ...
                    m = numpy.multiply(m1,m2**3)/(max_possible_map_value1*max_possible_map_value2**3)
                elif False:
                    m, max_possible_map_value = self.map(eventid, 'all', plot_map=False, plot_corr=False, hilbert=hilbert, mollweide=False, zenith_cut_ENU=zenith_cut_ENU, return_max_possible_map_value=True)
                else:
                ...
                total_max_corr_values = numpy.maximum(total_max_corr_values, m)

            map_max_phis = numpy.asarray(map_max_phis)
            map_max_thetas = numpy.asarray(map_max_thetas)
            ...
            if plot_map:
                ...
                # if mollweide:
                #     plt.scatter(map_max_phis, numpy.pi/2 - map_max_thetas, c='g', s=10)
                # else:
                #     plt.scatter(map_max_phis, 90.0 - map_max_thetas, c='g', s=10)

                ax.legend(loc='lower left')
                self.figs.append(fig)
                self.axs.append(ax)
                return total_max_corr_values, map_max_phis, map_max_thetas, fig, ax
            else:
                return total_max_corr_values, map_max_phis, map_max_thetas
        except Exception as e:
            print('\nError in %s'%inspect.stack()[0][3])
            print(e)
            exc_type, exc_obj, exc_tb = sys.exc_info()
