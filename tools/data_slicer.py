...
class ParameterFunction():
    '''
    Can be generated by a dataSlicerSingleRun and used as a way to process arbitrary functions on parameters when
    loading. 
    ...
    def __init__(self, ds, name, param_keys, func, plot_label, min_bin_val, max_bin_val, n_bins, description=''):
        ...
        self.bin_edges = numpy.linspace(self._min_bin_val,self._max_bin_val,self._n_bins + 1) #These are bin edges

    def getDataFromParam(self, eventids):
        try:
            params = [None]*len(self._param_keys)

            for i, param_key in enumerate(self._param_keys):
                params[i] = self._ds.getDataFromParam(eventids, param_key)

            params = tuple(params)
            return self._func(params)
        except Exception as e:
            print('\nError in %s'%inspect.stack()[0][3])
            print('Run: ',self._ds.reader.run)
            print(e)
...
class dataSlicerSingleRun():
    ...
    def addParameterFunction(self, name, param_keys, func, plot_label, min_bin_val, max_bin_val, n_bins, description=''):
        ...
        func(   getDataFromParam(self, eventids, param_key[i]),
                getDataFromParam(self, eventids, param_key[i+1]),
                ...
    ...
    def getDataFromParam(self, eventids, param_key):
        ...
        try:
            ...
            if len(eventids) > 0:
                ...
                elif param_key in list(self.parameter_functions.keys()):
    ...
    def getSingleParamPlotBins(self, param_key, eventids, verbose=False):
        ...
        try:
            ...
            if param_key in list(self.parameter_functions.keys()):
                label = self.parameter_functions[param_key].label
                current_bin_edges = self.parameter_functions[param_key].bin_edges
                calculate_bins_from_min_max = False

            elif len(numpy.where([kw in param_key for kw in self.math_keywords])[0]) == 1:
                if verbose:
