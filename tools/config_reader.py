...
def generateConfigFromDeployIndex(outpath, deploy_index):
    ...
    if type(deploy_index) == int:
        ...
        configWriter(json_path, origin, antennas_physical, antennas_phase_hpol, antennas_phase_vpol, cable_delays, description="An autogenerated calibration file for using the existing values for deploy_index = %i"%deploy_index,update_latlonel=True,force_write=True)
    else:
