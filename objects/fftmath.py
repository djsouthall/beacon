...
def handle_errors(func):
    '''
    A decorator function that can be used to provide some simple error handling.
    '''
    def func_wrapper(*args, **kwargs):
        try:
           return func(*args, **kwargs)
        except Exception as e:
            function_name = str(func.__class__) + '.' + str(func.__name__)
            print('Error in %s'%function_name)
            print(e)
...
class FFTPrepper:
    ...
    @handle_errors
    def calculateGroupDelays(self, t, wfs, plot=False,event_type=None):
        '''
...
if __name__ == '__main__':
    try:
        ...
        group_delay_freqs, group_delays = tct.calculateGroupDelaysFromEvent( eventids['hpol'][0], apply_filter=False, plot=True,event_type=None)
        ...
        vpol_waveforms = tct.averageAlignedSignalsPerChannel(eventids['vpol'], align_method=0, template_eventid=eventids['vpol'][0], plot=False)
